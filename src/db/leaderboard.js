...
// Борд зрителя по его полу: женщина ('f') → женский борд (мостик), все остальные
// (мужчины и не заданный пол NULL) → мужской борд (жим). Совпадает с серверным
// распределением участников (coalesce(sex,'m') <> 'f'). Используется на экране
// рейтинга, чтобы зритель видел ТОЛЬКО свой борд (мужской — мужчинам, женский —
// женщинам), а не оба сразу.
export function viewerBoard(sex) {
  return sex === 'f' ? 'f' : 'm'
}
