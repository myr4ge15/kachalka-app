...
// Состав для записи из отмеченных подходов: снятая галочка = «этого подхода не
// было», такой подход в документ не идёт. Упражнение без единого выполненного
// подхода выпадает целиком — ровно то же делает cleanEntries в db/repo.js с
// пустым составом. Формат подхода не меняется, отметки в документ не текут.
//
// Применяется ТОЛЬКО к правке уже сохранённой тренировки: она открывается «всё
// выполнено», поэтому снятая галочка там — явное отрицание. У новой тренировки
// отметок в начале нет вообще, их отсутствие ничего не значит, и на состав
// записи галочки не влияют (иначе «сохранить, не отметив» дало бы пустоту).
export function keepDoneSets(entries, doneKeys) {
  const kept = []
  for (const entry of entries ?? []) {
    const exId = entry?.exercise?.id
    const sets = (entry?.sets ?? []).filter((set, si) => isSetDone(doneKeys, exId, set, si))
    if (sets.length > 0) kept.push({ ...entry, sets })
  }
  return kept
}
